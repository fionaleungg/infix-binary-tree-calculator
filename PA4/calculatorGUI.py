...
# input: button input from user
# output: evaluates expression from user button input and returns the result of the evaluated infix expression
...
def clickButton(entrybox, value):
    ...
    if value == "c":
        ...
        inp = entrybox.get()
        entrybox.delete(0, END)
        entrybox.insert(0, str(calculate(inp)))
    else:
        entrybox.insert(100, value) 
